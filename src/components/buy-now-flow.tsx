...
import { formatRupees, parseRupeePrice } from "@/lib/cart"
...
export function BuyNowFlow({ product, quantity = 1 }: { product: Product; quantity?: number }) {
  ...
  function onSubmit(data: AddressFormValues) {
    ...
    const unitPrice = parseRupeePrice(product.price)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="lg" variant="outline" className="w-full transition-transform duration-300 hover:scale-105">
          Buy Now
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter your address to complete the purchase of <strong>{product.name}</strong>.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
